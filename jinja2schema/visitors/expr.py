...
def _visit_dict(ast, ctx, macroses, items, config=default_config):
    ...
    for key, value in items:
        value_rtype, value_struct = visit_expr(
            value,
            Context(
                predicted_struct=Unknown.from_ast(
                    value, order_nr=config.ORDER_OBJECT.get_next()
                )
            ),
            macroses,
            config=config,
        ...
        if isinstance(key, nodes.Node):
            key_rtype, key_struct = visit_expr(
                key,
                Context(
                    predicted_struct=Scalar.from_ast(
                        key, order_nr=config.ORDER_OBJECT.get_next()
                    )
                ),
                macroses,
                config=config,
...
def visit_compare(ast, ctx, macroses=None, config=default_config):
    ...
    rtype, struct = visit_expr(
        ...
        Context(
            predicted_struct=Unknown.from_ast(
                ast.expr, order_nr=config.ORDER_OBJECT.get_next()
            )
        ),
        macroses,
        config=config,
...
def visit_slice(ast, ctx, macroses=None, config=default_config):
    ...
    struct = visit_many(
        nodes, macroses, config, predicted_struct_cls=Number, return_struct_cls=Number
    )
...
def visit_name(ast, ctx, macroses=None, config=default_config):
    kwargs = {"order_nr": config.ORDER_OBJECT.get_next()}
    return (
        ctx.return_struct_cls.from_ast(ast, **kwargs),
        Dictionary({ast.name: ctx.get_predicted_struct(label=ast.name)}),
    )
...
def visit_getitem(ast, ctx, macroses=None, config=default_config):
    ...
    if isinstance(arg, nodes.Const):
        if isinstance(arg.value, int):
            if config.TYPE_OF_VARIABLE_INDEXED_WITH_INTEGER_TYPE == "list":
                ...
                predicted_struct = Tuple.from_ast(
                    ast,
                    tuple(items),
            ...
            raise InvalidExpression(
                arg,
                "{0} is not supported as an index for a list or"
    ...
    rtype, struct = visit_expr(
        ast.node,
        Context(ctx=ctx, predicted_struct=predicted_struct),
        macroses,
        config=config,
    )
...
def visit_test(ast, ctx, macroses=None, config=default_config):
    ...
    rtype, struct = visit_expr(
        ast.node,
        Context(return_struct_cls=Boolean, predicted_struct=predicted_struct),
        macroses,
        config=config,
    )
    if ast.name == "divisibleby":
        ...
        _, arg_struct = visit_expr(
            ast.args[0],
            Context(
                predicted_struct=Number.from_ast(
...
def visit_concat(ast, ctx, macroses=None, config=default_config):
    ctx.meet(Scalar(), ast)
    return (
        String.from_ast(ast, order_nr=config.ORDER_OBJECT.get_next()),
        visit_many(ast.nodes, macroses, config, predicted_struct_cls=String),
    )
...
def visit_cond_expr(ast, ctx, macroses=None, config=default_config):
    ...
    test_rtype, test_struct = visit_expr(
        ast.test,
        Context(predicted_struct=test_predicted_struct),
        macroses,
        config=config,
    )
        ...
        if macroses and ast.node.name in macroses:
            ...
            if call.passed_args or call.expected_args:
                raise InvalidExpression(
                    ast,
                    (
                        'incorrect usage of "{0}". it takes '
                        "exactly {1} positional arguments".format(
                            macro.name, len(macro.args)
                        )
                    ),
                )
            if call.passed_kwargs:
                ...
                raise InvalidExpression(
                    ast,
                    (
                        'incorrect usage of "{0}". unknown keyword argument '
            ...
            for kwarg in ast.kwargs:
                arg_rtype, arg_struct = visit_expr(
                    ...
                    Context(
                        predicted_struct=Scalar.from_ast(
                            kwarg, order_nr=config.ORDER_OBJECT.get_next()
                        )
                    ),
                    macroses,
                    config=config,
            ...
            return _visit_dict(
                ...
                ctx,
                macroses,
                [(kwarg.key, kwarg.value) for kwarg in ast.kwargs],
                config=config,
            )
        else:
            raise InvalidExpression(
...
def visit_filter(ast, ctx, macroses=None, config=default_config):
    ...
    if ast.name in (
        ...
        rtype = List(
            List(Unknown(), linenos=[ast.node.lineno]), linenos=[ast.node.lineno]
        )
    ...
    rv = visit_expr(
        ast.node,
        Context(
            ctx=ctx, return_struct_cls=return_struct_cls, predicted_struct=node_struct
        ),
        macroses,
        config=config,
    )
...
def visit_tuple(ast, ctx, macroses=None, config=default_config):
    ...
    rtype = Tuple.from_ast(
        ast, item_structs, constant=True, order_nr=config.ORDER_OBJECT.get_next()
    )
    return rtype, struct
...
def visit_list(ast, ctx, macroses=None, config=default_config):
    ...
    for item in ast.items:
        item_rtype, item_struct = visit_expr(
            item, Context(predicted_struct=predicted_struct), macroses, config=config
        )
        struct = merge(struct, item_struct)
        if el_rtype is None:
            el_rtype = item_rtype
...
def visit_dict(ast, ctx, macroses=None, config=default_config):
    ...
    return _visit_dict(
        ...
        ctx,
        macroses,
        [(item.key, item.value) for item in ast.items],
        config=config,
    )
