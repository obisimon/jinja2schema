...
class MacroCall(object):
    def __init__(self, macro, passed_args, passed_kwargs, config=default_config):
        ...
        for arg_ast in passed_args:
            arg_rtype, arg_struct = visit_expr(
                arg_ast,
                Context(predicted_struct=Unknown.from_ast(arg_ast)),
                config=config,
            )
        ...
        for kwarg_ast in passed_kwargs:
            kwarg_rtype, kwarg_struct = visit_expr(
                kwarg_ast.value,
                Context(predicted_struct=Unknown.from_ast(kwarg_ast)),
                config=config,
            )
    ...
    def _match_passed_args(self, to_args):
        ...
        for i, ((arg_ast, arg), (expected_arg_name, expected_arg)) in enumerate(
            matched_args, start=1
        ):
            _, s = visit_expr(
                arg_ast,
                Context(predicted_struct=merge(arg, expected_arg)),
                config=self.config,
            )
            rv = merge(rv, s)
    ...
    def _match_passed_kwargs(self, to_args):
        rv = Dictionary()
        for kwarg_name, (kwarg_ast, kwarg_type) in list(
            _compat.iteritems(self.passed_kwargs)
        ):
            for (expected_arg_name, expected_arg_struct) in list(to_args):
                if kwarg_name == expected_arg_name:
                    _, s = visit_expr(
                        kwarg_ast.value,
                        Context(
                            predicted_struct=merge(kwarg_type, expected_arg_struct)
                        ),
                        config=self.config,
                    )
                    rv = merge(rv, s)
